...
public class Menu {
    ...
    public static int getBaseChoice(String type) {
        return Inputter.getAnInteger(
            String.format("Choose the base number of the %s value: \n" +
                          ...
                          "Enter your choice (1-3):", type),
            "Invalid choice! Please enter 1, 2, or 3.", 1, 3);
    }
    ...
    public static int getOutputBaseChoice(int inputBase) {
        ...
        if (inputBase != 1) baseOptions += "1. Binary (Base 2)\n";
        if (inputBase != 2) baseOptions += "2. Decimal (Base 10)\n";
        if (inputBase != 3) baseOptions += "3. Hexadecimal (Base 16)\n";
        baseOptions += "Enter your choice:";

        int outputBase;
        while (true) {
            outputBase = Inputter.getAnInteger(baseOptions, 
                "Invalid choice! Please enter one of the available options.",1,3);

            // Ensure output base is different from input base
            if (outputBase != inputBase) break;
